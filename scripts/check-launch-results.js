...
【SEO・タイトル・description の重要ルール】
- タイトルには必ず**日本語のロケット名**を含める（例: 「ファルコン9」「アリアン6」「H3」「長征2号C」「ソユーズ2」「エレクトロン」「朱雀2号」）
- タイトルの形式: 「{日本語ロケット名}、{ミッション内容}に成功/失敗」（35文字以内）
- 英語名が一般的に使われる場合（H3, Starship等）はそのままでOK
- descriptionには「{日本語ロケット名} 打ち上げ {成功/失敗} {年月}」を必ず含める
- descriptionの形式: 「{ロケット名}が{何を}{どうした}。{射場}から{年月日}に実施。」（90文字以内）
- 主要ロケットの日本語表記: Falcon 9→ファルコン9, Falcon Heavy→ファルコンヘビー, Ariane 6→アリアン6, Electron→エレクトロン, Soyuz→ソユーズ, Long March→長征, Zhuque→朱雀, GSLV→GSLV, PSLV→PSLV, Vega→ヴェガ, New Glenn→ニューグレン, Vulcan→ヴァルカン
${isUnknownPayload ? `
【ペイロード不明時の注意】
